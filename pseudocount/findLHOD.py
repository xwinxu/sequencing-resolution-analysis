...
"""
find_LOD: finds the limit of detection, or the smallest non-zero value, from a genomedata archive in order to add the asinh(LOD/2) as a pseudocount to trained means.
"""

import sys
from argparse import ArgumentParser
import numpy
from numpy import (amin, amax, argmax, array, diff, hstack, isfinite, NINF, PINF, square)
from tables import NoSuchNodeError
from genomedata import Genome, __version__
from genomedata._load_seq import MIN_GAP_LEN
from genomedata._util import fill_array, init_num_obs, new_extrema
...
def trim_chunks_start(prev_supercontig, prev_end_coord,
    ...
    if prev_supercontig:
        ...
        if (first_present_value_coord - prev_end_coord > MIN_GAP_LEN):
...
def trim_chunks_end(next_supercontig, next_start_coord,
    ...
    # NB: argmax will give the index of the first element on matching
    # winners of the boolean array. The winners are "present" values
    last_present_index = (len(presence_mask) - argmax(presence_mask[::-1]))

    # If this is not the last supercontig
    if next_supercontig:
        last_present_chrom_coord = (supercontig.start + last_present_index)
        # If the next present value is greater than MIN_GAP_LEN away
        # or there is no next possible present value
        if (not next_start_coord or next_start_coord - last_present_chrom_coord > MIN_GAP_LEN):
...
def write_metadata(chromosome, verbose=False):
    ...
    for prev_supercontig, supercontig, next_supercontig in zipper:
        ...
        next_chrom_start = find_next_present_coord(chromosome, next_supercontig)
        ...
        if any(mask_rows_any_present):
            ...
            starts[0] = trim_chunks_start(prev_supercontig, prev_chrom_end, supercontig, mask_rows_any_present, starts[0])

            # Trim the end chunk index for this supercontig
            ends[-1] = trim_chunks_end(next_supercontig, next_chrom_start, supercontig, mask_rows_any_present, ends[-1])
...
def write_genome_metadata(genome, verbose):
    ...
    for chromosome in genome:
        if chromosome.attrs.dirty:
            (mins, maxs) = write_metadata(chromosome, verbose=verbose)
            mini = get_nonzero_min(mins,0)
            tempmin = numpy.concatenate((tempmin, numpy.array([mini])))
            maxi = maxs.max()
            tempmax = numpy.concatenate((tempmax, numpy.array([maxi])))
    #print out the minimum after looping through all chromosomes
    print((tempmin.min(), tempmax.max()))
